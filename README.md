...
# Production build (regenerates public/sw.js with a fresh VERSION)
pnpm build
pnpm start
```
...
app/
  ...
  robots.ts                 /robots.txt (allow all, disallow /api & /offline)
  sitemap.ts                /sitemap.xml — static + 16 exhibitions + 10 galleries
  opengraph-image.tsx       1200×630 OG card generated at the edge
  icon.tsx                  512×512 PNG favicon (Next 14 convention)
...
lib/
  ...
  routes.ts                 Typed-route helpers (galleryHref, exhibitionHref...)
...
public/
  ...
  icons/                    SVG app icons (manifest)
  sw.js                     Service Worker (generated — see scripts/)

scripts/
  sw.template.js            Service Worker source (committed)
  build-sw.mjs              Generator: injects VERSION into public/sw.js

tests/
  unit/                     Vitest unit tests (schemas, format, api, routes)
  stubs/                    'server-only' module stub for tests
```
...
- Icons: manifest points at `public/icons/*.svg` (`sizes: "any"`);
  HTML-level favicon (512 px) and apple-touch-icon (180 px) are PNG
  generated by Next 14 file conventions (`app/icon.tsx`,
  `app/apple-icon.tsx`).
- OG card at `/opengraph-image` (1200×630) from `app/opengraph-image.tsx`.
- Service Worker is **generated** at `public/sw.js` by
  `scripts/build-sw.mjs`, which injects a fresh `VERSION` from
  `$BUILD_ID` → `$VERCEL_GIT_COMMIT_SHA` → `git rev-parse --short HEAD`
  → timestamp fallback. Runs via `pnpm prebuild` and `pnpm predev`, so
  every build ships a new SW scope and cache generation.
- SW strategies (`scripts/sw.template.js`):
  - network-first for navigations, cache only 200/basic responses,
    fallback to cache, then `/offline`;
...
- `pnpm test` runs the Vitest unit suite under `tests/unit/`. Current
  coverage: Zod schemas, `getExhibitionStatus`, date/price formatters,
  `ExhibitionsQuerySchema` + `filterExhibitions`, and the `lib/routes.ts`
  helpers (47 tests).
- E2E / axe-core / Lighthouse runs are scoped to a Vercel Preview URL
  and tracked in `docs/qa-plan.md`.
